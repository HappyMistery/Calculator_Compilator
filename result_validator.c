...
int validate_results(const char *output_file) {
    ...
    double expected_arit_results[] = {
        ...
        (7.5 + 2.5) / 2.0 - 1.5 * 3, 
        (10.2 - 4.1) * 2.0 + 1.0 / 2.0, 
        3.0 * (4.0 + pow(2.5, 2.0)), 
        fmod(8.5, 3.2) + 4.2 * 1.5 - 3.0 / 1.5,
        ...
        (7 + 2.5) * 3 - 4 / 2 + 1.5, 
        10 - (3.5 * 2) + pow(8, 2) - 4 / 3.0,
        (3 * 2.0 + pow(1.5, 2)) - 7 / 3, 
        (fmod(5, 2.5) + 4.5) * 3 - 2 / 1.0,
        (30 + sin(30)) * 2 - 4 / 2, 
        10 - (3 * cos(30)) + 5 - 3,
        (7.5 + sin(M_PI / 4)) * 2 - 1.5, 
        10.2 - (3 * cos(M_PI / 4)) + 1.5,
    ...
    int index;
    int passed = 0;
    printf("%d aritmetic tests to pass...\n", total_results);
    ...
    passed = 0;
    printf("%d string concat tests to pass...\n", total_results);
    for (index = 0; index < total_results; index++) {
        fgets(buffer, 511, file);
